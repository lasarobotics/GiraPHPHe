// Tetrix Gear Source.scad
// Daniel Teal 2014
//
// Generates a 3D model of a gear sized for Tetrix parts
//
// Usage: change the variable gear_teeth to one's preference, render the design by pressing F6 (it'll take a while), then export as an STL file.
//
// 20 teeth is equivalent to a small Tetrix gear (40T)
// 40 teeth is equivalent to a medium Tetrix gear (80T)

// Edit this:
gear_teeth = 20;

// Source code: leave alone unless you want to experiment.

/* TETRIX GEAR */

difference(){
	rotate(36/8)gear(pitch_radius=0.8*gear_teeth, teeth=gear_teeth, height=7.5, helix_angle=-30);
	translate([0,0,-1]){
		cylinder(r=4.2,h=10);
		for(i=[0:3]){
